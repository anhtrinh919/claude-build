...
Write `specs/<phase>/design-brief.md` from `${CLAUDE_PLUGIN_ROOT}/skills/build/schemas/design-brief-internal.md`. **Invoke the `impeccable` skill to do the actual design work — let it work.** Its own judgment (register, color strategy, type hierarchy, layout pattern, anti-slop bans, motion) drives the `Visual style` / `Component decisions` / `Interaction patterns` sections; don't re-derive a bespoke craft ruleset here. North star at top under `## Design intent`. Skipping the brief because a design exists is the process violation — never skip it. Self-verify before continuing: visual style + scene sentence + screen checklist all present.
...
- **Craft (register, color, type, layout, anti-slop, motion): the `impeccable` skill, not a reference file.** Invoked at brief-writing time and kept in mind through M2/M3 — its judgment, not a restated ruleset here.
