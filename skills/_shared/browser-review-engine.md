# Browser Review Engine — shared by /dogfood + /review

The browser-driven review pattern shared by `/dogfood` (post-implementation gate outside `/build`) and `/review` Step 2 (UX dogfooding inside `/build`). Both run scenarios via a Sonnet subagent, run a blind naive-reviewer subagent, and gate completion on three signals.
...
Output per scenario: PASS / FAIL, with a one-line "expected vs seen" for any FAIL and a screenshot reference. Nothing else.
```
...
1. **Functional** — every scenario from Engine 1 passes.
...
- **Scope derivation.** `/dogfood` derives 2–4 scenarios from "what just changed"; `/review` walks every user story in `requirements.md`. Both then call Engine 1 with their scenario list.
- **The user-goal sentence.** `/dogfood` derives it from the original problem; `/review` extracts it from `requirements.md`. Both pass it verbatim to Engine 2.
- **Issue aggregation and reporting.** Each caller has its own report shape (severity tables, health score, etc.). The engine produces raw findings; the caller merges them into its report.
- **State-file writes** (`.build-state.json`). Only `/review` writes state — `/dogfood` runs outside `/build` and has no state file.
- **Iteration counter persistence.** `/review` reads/writes `reviewIteration` in `.build-state.json`; `/dogfood` tracks it in-conversation only.
