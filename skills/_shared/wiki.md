# Wiki integration — shared across SDD skills

Non-blocking. Failures log and continue — wiki is best-effort, never a gate.

Each skill that loads this reference passes its own `$AGENT` (e.g. `spec`, `frontend`, `backend`, `review`, `ba`) and its `$TAGS` (derived from `tech-stack.md`, up to 5 — e.g. `nextjs,tailwind,bun`). Mode 1 of `/spec` has no `tech-stack.md` yet — pass `$TAGS` empty.
...
```
node "${CLAUDE_PLUGIN_ROOT}/scripts/wiki.mjs" read --agent $AGENT --tags "$TAGS" --limit 5
```

- If output starts with `# <N> relevant entries`: index the titles + one-line hooks under `## Relevant past learnings` in the skill's working context. Do NOT inline full bodies — read on demand if a hit looks useful.
- If output is `# No relevant entries` or the CLI fails (non-zero exit): log `No prior $AGENT learnings` and continue silently. Never block.
...
- Do not save a friction entry for the same issue twice in one phase (the wiki dedup keys on title+tags, but quality bar — be intentional).
