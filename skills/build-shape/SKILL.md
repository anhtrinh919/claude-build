...
Apply `${CLAUDE_PLUGIN_ROOT}/skills/build/_shared/drilling-discipline.md` in **`concept-shape`** mode — root→trunk→branch→leaf, gated at each level (structural + semantic), one thread at a time. This gate is usually shallow: most ideas resolve at root+trunk+branch, rarely reaching a leaf here (that depth belongs to Step 1.4) — a shallow resolution is the tree being correct, not incomplete.

- **Root — is this worth building at all?** Demand, walked live via `AskUserQuestion`, taking a position on every answer ("that could work" is banned): strongest evidence someone wants this (push until you hear someone paid, was upset when it broke, built a workflow around the problem — red flags: waitlist signups, VC interest, "sounds useful"); what people do right now, even badly (red flag: "nothing" means the problem isn't painful enough). **Fast fatal check here, before opening the next level:** if a lens fires fatal (a `~/.claude/CLAUDE.md` contradiction, ≥80% existing-skill overlap the user can't distinguish, or no problem nameable after 2–3 rounds), surface it as a blocking concern now — skip straight to Step 1.3's gate rather than drilling further on a doomed idea.
- **Trunk — who needs this most, specifically?** The actual person (red flag: "enterprises"/"marketing teams" are filters, not people) — the actor whose need justifies the root. Skip straight to branch if the idea already names someone specific enough that nothing forks on it.
- **Branch — the remaining big-shape either/or decisions**, only those the idea hasn't already settled, from this closed vocab: **single-vs-multi** (one workspace or shop / many) · **spine** (the one core job — and which of two plausible spines) · **boundary** (a tool / a content surface / a pipeline). Each fork is a real decision — two genuinely different products, never a fake choice — carrying its options + plain-language tradeoff + a recommended default (`voice.md` fork form), surfaced via `AskUserQuestion`, 2–5 forks.
- **Leaf** — details inside an already-settled branch, only where genuinely ambiguous; most ideas need none here (that depth belongs to Step 1.4).
