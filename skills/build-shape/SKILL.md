...
Apply `${CLAUDE_PLUGIN_ROOT}/skills/build/_shared/drilling-discipline.md` for question craft — batched `AskUserQuestion`, a recommendation on every question, take a position, research before recommending. Its **four-step shape does not apply here**: this gate carries only a handful of forks, so it resolves in batched orientation alone. No `decision-tree.md`, no artifact — that depth belongs to Step 1.4. Walk the trunk/branch/leaf structure below, not a fixed script.
