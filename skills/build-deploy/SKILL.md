...
Write `currentSubStep: "deploy.3.4"`. Read `deploy_platform` from `tech-stack.md ## Safety Defaults`. `TBD`/absent → ask once via `AskUserQuestion` (same four options as SD5 — Vercel / Railway / Render / Other), then patch `tech-stack.md`'s SD5 value with the answer so a re-deploy doesn't ask again.
...
**Other — fully guided.** Write the generic `docs/deployment.md` (env vars, build command, start command, health-check endpoint if the `safety` baseline is active) and stop there.
