...
|---|---|---|---|---|---|
| **pipeline-review** | Opus main; `code-reviewer` + `dogfood` agents; Opus fix leaves | inline | spec dir, `validation.md`, `outcome-card.md`, `requirements.md`, running app | review report + coverage briefing + silent fixes + dogfood handoff | **`phase-complete`** (convergence or Accept) / **`phase-blocked`** (Stop) — this skill writes it |
