...
- **Each group has a verify line.** What command proves it's done (e.g. `tsc -p . --noEmit`, `bun test --run CardDetail`, `verify-group-N.sh`). Groups are built in dependency order; a group whose verify already passes is skipped (resume-idempotency).
