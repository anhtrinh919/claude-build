...
- The orchestrator **is the session**: it runs each sub-skill **inline**, in order — never spawns one as a subagent (mechanism + leaf-worker list: "Model + mechanism" below).
...
- **Roadmap discipline:** Phase 0 = Foundation (shell + hero screens, polished static, unwired); Phases 1+ = vertical slices, slice-tested, no horizontal phases. User is the PM; build-spec drafts the sequence, user confirms. Full doctrine + axis-before-order: `_shared/roadmap-axis.md` — governs Step 1.6's roadmap draft (build-spec constitution mode).
